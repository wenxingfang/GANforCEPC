...
if __name__ == '__main__':
    ...
    input2 = mc_info[iBatch*parse_args.batch_size:(iBatch+1)*parse_args.batch_size, 3:6]

    result = model.predict([input1, input2], verbose=True)
    ...
    real[:,0]   = real[:,0]*49.5 + 50.5
    real[:,1]   = real[:,1]*50 + 90
    real[:,2]   = real[:,2]*20
#    real[:,3]   = real[:,3]*10
#    real[:,4]   = real[:,4]*10
#    real[:,5]   = real[:,5]*2000
    result[:,0] = result[:,0]*49.5 + 50.5
