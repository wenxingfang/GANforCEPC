...
# -*- coding: utf-8 -*-
""" 
useBFiled, use cell ID for ECAL. Adding HCAL
target: mom, dtheta, dphi
input ECAL: 31,31,29
input HCAL: 40 ,40, 55
add validation part
"""
...
def get_parser():
    ...
    parser.add_argument('--doValid', action='store', type=ast.literal_eval, default=True,
                        help='do Valid')
    parser.add_argument('--earlyStopEpoch', action='store', type=int, default=5,
                        help='early stop epach')

    parser.add_argument('--datafile', action='store', type=str,
                        help='yaml file with particles and HDF5 paths (see '
                        'github.com/hep-lbdl/CaloGAN/blob/master/models/'
                        'particles.yaml)')
    parser.add_argument('--validfile', action='store', type=str,
                        help='validfile')
...
if __name__ == '__main__':
    ...
    earlyStopEpoch = parse_args.earlyStopEpoch
    ...
    features_H = build_regression_v1(image=calorimeter_H, epsilon=0.001)
    print('features_H:',features_H.shape)
    energies_H = calculate_energy(calorimeter_H)
    print('energies_H:',energies_H.shape)
    tot_en = add([energies, energies_H])
    print('tot energies:',tot_en.shape)
    ...
    #p = concatenate([features, energies, pos_info])
    p = concatenate([features, energies, features_H, energies_H, tot_en, pos_info])
    '''
    p = concatenate([
        features,
        scale(energies, 10),
    ...
    regression = Model([calorimeter, calorimeter_H, pos_info], regression_outputs)
    ...
    logger.info('preparing data list for training')
    ...
    ##################################################
    logger.info('preparing data list for validing')
    Data_v  = []
    Event_v = []
    Batch_v = []
    if doValid:
        f_DataSet_v = open(parse_args.validfile, 'r')
        for line in f_DataSet_v: 
            idata = line.strip('\n')
            idata = idata.strip(' ')
            if "#" in idata: continue ##skip the commented one
            Data_v.append(idata)
            ...
            ievent   = d['Barrel_Hit'].shape[0]
            d.close()
            Event_v.append(float(ievent))
            Batch_v.append(int(float(ievent)/batch_size))
        total_event_v = sum(Event_v)
        f_DataSet_v.close() 
        print('total valid sample:', total_event_v)
        print('All valid Batch:', Batch_v)
    #################################################################
    cost_list = []
    ...
    for epoch in range(nb_epochs):
        ...
        for ib in range(len(Batch)):
            ...
            for index in range(ibatch):
                ...
                info_batch_v1 =  mc_info[index * batch_size:(index + 1) * batch_size, 3:7]
                ...
                real_batch_loss = regression.train_on_batch(
                    [image_batch, image_H_batch, info_batch_v1],
                    reg_outputs_real,
                    loss_weights
                )
        ...
        if (epoch+1)%10 == 0:
            out_name = (parse_args.model_out).replace('.h5','_%d.h5'%(epoch+1))
            regression.save(out_name)
        ################
        if doValid:
            epoch_loss_v = []
            processed_batch = 0
            for ib in range(len(Batch_v)):
                first, second, mc_info = load_data(Data_v[ib])
                ibatch = Batch_v[ib]
                for index in range(ibatch):
                    ...
                    real_batch_loss = regression.test_on_batch(
                        [image_batch, image_H_batch, info_batch_v1],
                        reg_outputs_real,
                        loss_weights
                    )
                    if math.isnan(real_batch_loss): continue
                    epoch_loss_v.append(np.array(real_batch_loss))
                #processed_batch = processed_batch + ibatch
                #logger.info('processed {}/{} total batches'.format(processed_batch, nb_batches))
            logger.info('Epoch {:3d}  valid loss: {}'.format( epoch + 1, np.mean(epoch_loss_v, axis=0)))
            ############ early stop ###################
            avg_cost = np.mean(epoch_loss_v, axis=0)
            if len(cost_list) < earlyStopEpoch: cost_list.append(avg_cost)
            else:
                for ic in range(len(cost_list)-1):
                    cost_list[ic] = cost_list[ic+1]
                cost_list[-1] = avg_cost
            if epoch > earlyStopEpoch and avg_cost >= cost_list[0]: 
                out_name = (parse_args.model_out).replace('.h5','_%d.h5'%(epoch+1))
                regression.save(out_name)
                logger.info('EarlyStop. Epoch {:3d}  final valid loss: {}'.format( epoch + 1, avg_cost))
                break
