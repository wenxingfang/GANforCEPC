...
def normalize(x, epsilon):
    print('norm x shape=',x.shape)
    assert len(x.shape)==5
    x_mean, x_variance = tf.nn.moments(x, axes=[1,2,3,4], shift=None, keepdims=True, name=None)
...
def minibatch_discriminator(x):
    ...
    print('x shape 2:',(K.permute_dimensions(x, [1, 2, 0])).shape)
    print('x shape 3:',( K.expand_dims(K.permute_dimensions(x, [1, 2, 0]), 0) ).shape)
