...
if __name__ == '__main__':
    ...
    if parse_args.exact_model:
        ...
        index_line=0
        for line in f_info:
            (mom, M_dtheta, M_dphi, P_dz, P_dy, Z) = line.split(',')
            mom      = (float(mom.split('=')[-1])-50.5)/49.5
    ...
    if parse_args.comb_model_in !='':
        ...
        results[1][:,0] = results[1][:,0]*49.5 + 50.5
        results[1][:,1] = results[1][:,1]*50 + 90
        results[1][:,2] = results[1][:,2]*20
        results[1][:,3] = results[1][:,3]*10
        results[1][:,4] = results[1][:,4]*10
        results[1][:,5] = results[1][:,5]*2000
    ...
    if parse_args.check_dis_real and parse_args.dis_model_in !='':
        ...
        if parse_args.for_mc_reco:
            dis_input = [real_input, mc_info[:,0:6]]
        d.close()
