...
SLCIORdr::SLCIORdr(string name, string output, string mapTxt){
...
bool SLCIORdr::mutate(){
    ...
    if(lcEvent) {  //cout<<"det name="<< lcEvent->getDetectorName() <<endl;
                   ...
                   // std::cout<<colVec->at(i)<<std::endl;
                   //}
                   for(unsigned int i=0; i< colVec->size(); i++){
                       if (colVec->at(i)=="MCParticle")
                       {
                           Col = lcEvent->getCollection("MCParticle");
                           LCCollection* HitCol  = lcEvent->getCollection("EcalBarrelSiliconCollection");
                           ...
                           //std::cout<<"idencoder="<<ecalinitString<<std::endl;
                           m_Hit_map.clear();
                           //check time
                           /*
                           ...
                           */
                           ...
                           //LCCollection* calHits = new LCCollection( LCIO::CALORIMETERHIT )  ;
                           ...
                           map<int, vector<float> >::iterator map_iter = m_Hit_map.begin();
	                   for(; map_iter!=m_Hit_map.end(); map_iter++)
                           {
                               CalorimeterHitImpl* calHit = new CalorimeterHitImpl ;
	                       float pos[3] = { (map_iter->second).at(1) , (map_iter->second).at(2), (map_iter->second).at(3) } ;
                               float time0 =0.0001+(1.0/300)*sqrt(pos[0]*pos[0] + pos[1]*pos[1] + pos[2]*pos[2]);
                               calHit->setTime(time0);
                               calHit->setCellID0(   map_iter->first ) ;
                               calHit->setCellID1(0) ;
                               calHit->setEnergy( (map_iter->second).at(0) ) ;
                               calHit->setPosition(pos) ;
                               calHits->addElement( calHit ) ;
                               //std::cout<<"e="<<hit_e[j]<<",x="<<hit_x[j]<<",y="<<hit_y[j]<<",z="<<hit_z[j]<<",id="<<hit_id[j]<<std::endl;
                           }
                           lcEvent->addCollection( calHits, "ECALBarrel") ; 
                           _lcWrt->writeEvent(lcEvent) ;
                           //delete calHits;// can't delete ?
                           break;
                       }// if mc particle
                   }
                  }
...
void SLCIORdr::MakeMap(const vector<float>& hit_e, const vector<int>& hit_id, const vector<float>& hit_x, const vector<float>& hit_y, const vector<float>& hit_z)
{
    ...
    {
        vector<float> tmp;
        if(hit_id.at(i)==0 || hit_e.at(i)<1e-5) continue;
        int id = hit_id.at(i);
